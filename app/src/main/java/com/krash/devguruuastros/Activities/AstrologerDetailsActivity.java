...
import com.krash.devguruuastros.Adapters.PhotoListAdapte;
import com.krash.devguruuastros.Adapters.ReviewAdapter;
...
public class AstrologerDetailsActivity extends AppCompatActivity {
    ...
    String userType;
    private TextView name, degree, exp, lang, chat, call, report, bio, rating, busy, astOff, rate_average_rating;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rate_average_rating = findViewById(R.id.rate_average_rating);
                ...
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot snapshot1 : snapshot.getChildren()) {
                            ...
                            try {
                                reviewAcg = reviewAcg + Integer.parseInt(reviewModel.getRating());

                                rate_average_rating.setText(""+(float)(reviewAcg/reviews.size()));

                                report.setText(""+reviews.size()+" rating");

//                                report.setText(""+(reviewAcg/reviews.size())+" rating");
                                //Toast.makeText(AstrologerDetailsActivity.this, ""+(reviewAcg/reviews.size()), Toast.LENGTH_SHORT).show();
                ...
                public void onClick(View v) {
                    ...
                    if (model.getChatOnline().equalsIgnoreCase("Online") && model.getIsBusy().equalsIgnoreCase("false")) {
                        if (isAstrologerOnline() == 1) {
                            if (balance < (astPrice * 1)) {
                                binding.currentBalance.setText("Current Balance: ₹" + balance);
                                binding.textView36.setText("Minimum Wallet Balance required to talk is ₹" + (astPrice * 1) + ", Please recharge your wallet.");
                        ...
                        Toast.makeText(AstrologerDetailsActivity.this, "Astrologer is offline", Toast.LENGTH_SHORT).show();
                    }
                }
            });
            ...
            callBtn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (model.getCallOnline().equalsIgnoreCase("online") && model.getIsBusy().equalsIgnoreCase("false")) {
                        if ((balance >= 0) && (balance < (astCallPrice * 1))) {
                            binding.currentBalance.setText("Current Balance: ₹" + balance);
                            binding.textView36.setText("Minimum Wallet Balance required to talk is ₹" + (astCallPrice * 1) + ", Please recharge your wallet.");
                            binding.button4.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    Intent i = new Intent(AstrologerDetailsActivity.this, WalletActivity.class);
...
//                        dialog.show();
                        ...
                        if (model.getIsBusy().equalsIgnoreCase("true")) {
                            Toast.makeText(AstrologerDetailsActivity.this, "Astrologer is busy", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(AstrologerDetailsActivity.this, "Astrologer is offline", Toast.LENGTH_SHORT).show();
                        }
...
//                Toast.makeText(AstrologerDetailsActivity.this, "noti", Toast.LENGTH_SHORT).show();
    ...
    private void callEnded() {
        ...
        try {
            ...
            Log.d("user duration cover",""+usedDuration);
        } catch (Exception e) {
        }
        ...
        astMap.put("userpaid", ((double)(usedDuration * astCallPrice)) + "");

        double total =  ((double)(usedDuration * astCallPrice) / 2);
        //Toast.makeText(this, ""+total, Toast.LENGTH_SHORT).show();
        astMap.put("earning",  ""+total);
...
//                    Toast.makeText(AstrologerDetailsActivity.this, "", Toast.LENGTH_SHORT).show();
    ...
    public void onBackPressed() {
        ...
        if(userType.equalsIgnoreCase("ast"))
        {
            Intent i = new Intent(getApplicationContext(),AstrologerMainActivity.class).putExtra("comesfrom","astrodetail");
            startActivity(i);
            finishAffinity();
        }
        else
